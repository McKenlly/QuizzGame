...
router.delete(
  ...
  catchErrors(async (req, res) => {
    req.room.ended = true;
    await req.room.save();
    res.json({ message: 'Комната была закрыта.' });
  })
);
//#endregion
...
router.post(
  ...
  catchErrors(async (req, res) => {
    ...
    if (roomClosed) {
      return res.status(404).json({ message: 'Комната была закрыта' });
    }

    if (!name || name.length > 12) {
      return res.status(404).json({ message: 'Неверное имя' });
    }
    ...
    if (
      ...
      return res.status(404).json({ message: 'Команда уже играет' });
    }
    ...
    req.session.save(() => res.json({ message: 'Командная заявка была получена.' }));
  })
);
...
router.delete(
  ...
  catchErrors(async (req, res) => {
    ...
    res.json({ message: 'Заявка была отклонена.' });
  })
);
//#endregion
...
router.post(
  ...
  catchErrors(async (req, res) => {
    ...
    res.json({ message: 'Команда была утверждена.' });
  })
);
...
router.patch(
  ...
  catchErrors(async (req, res) => {
    ...
    if (!team) {
      return res.status(404).json({ message: 'Данного участника не существует' });
    }
    ...
    switch (req.session.role) {
      ...
      case TEAM:
        if (req.room.questionClosed) {
          return res.status(400).json({ message: 'Вопрос закрыт.' });
        }

        if (req.sessionID !== team.sessionID) {
          return res.status(404).json({ message: 'Это не ваша команда!' });
        }
        ...
        return res.json({ message: 'Предположение представлено!' });
      default:
        return res.status(400).json({ message: 'Вы не имеете права выполнять это действие.' });
    }
  })
);
...
router.post(
  ...
  catchErrors(async (req, res) => {
    ...
    req.session.save(() => res.json({ message: 'Вы были зарегистрированы.' }));
  })
);
//#endregion
