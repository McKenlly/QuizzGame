...
Room.methods.startRound = async function(categories) {
  try {
    if (this.roundStarted) {
      return Promise.reject({ statusCode: 400, message: 'Раунд уже был начат' });
    }

    if (categories.length !== 3) {
      return Promise.reject({ statusCode: 400, message: 'Невалидное количество категорий выбрано' });
    }
...
Room.methods.startQuestion = async function(question) {
  try {
    if (!this.questionClosed) {
      return Promise.reject({ statusCode: 400, message: 'Вопрос уже решается.' });
    }

    if (this.askedQuestions.includes(question._id)) {
      return Promise.reject({
        statusCode: 400,
        message: 'Выбранный вопрос уже был задан.',
      });
    }
