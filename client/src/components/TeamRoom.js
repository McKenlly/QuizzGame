...
const TeamRoom = () => {
  ...
  return isLoading || !open ? (
    <>
      <Loader /> {open && <Button onClick={handleChangeAnswer}>Изменить ответ</Button>}
    </>
  ) : (
    <>
      <span className="round-number">{`Раунд ${roundNo}`}</span>
      <span className="question-number">{`Вопрос ${questionNo}`}</span>
      <span className="category">{category}</span>
      <span className="question">{question}</span>
      <Input reducer="teamApp" item="guess" placeholder="Ваш ответ" labelText="Ответ" maxLength="50" showCounter />
